...
- 入口污点参数：**位置**（0-based）+ 签名 + 名字。
  - 若提示词标注“自行分析”，则 EA 未指定具体污点参数，你需要：
    1. 逐个分析所有入参，判断哪些携带外部输入
    2. 检查函数内部是否有产生外部数据的调用（如 `recv`/`read`/`fread`/`open`/`fetch`/网络 IO 等）
    3. 将所有识别到的污点源填入 `taints[]`
- 前置校验链：从根函数到本函数，调用链上已累积的校验（condition+content 列表）。

## 污点源识别规则

污点源不仅限于入参，以下都是合法的污点源：
- **入参**：函数参数中携带外部输入的变量（如网络报文指针、文件路径、用户输入等）
- **内部调用产物**：函数内部调用外部接口获取的数据（如 `msg = recv()` 返回的 `msg`）
- **被动传递**：函数可能不接收污点参数，但通过内部调用获取外部数据并返回
  （如 `A() { msg = recv(); return msg; }` 中 `msg` 是污点源）
