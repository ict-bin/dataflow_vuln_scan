...
def resolve_external(
    ...
    system_prompt = (v2_system + "\n\n" if v2_system else "") + (
        ...
        "6. 攻击面复核要求: 获取污点的下游函数的前提，都要根据函数的功能，根据污点值的含义，先判断污点以及逃逸的内容属于外部攻击者可控制的内容；对攻击者不可控的变量、进程内部状态、纯内部派生值以及其他无法控制的内容，以及不太可能造成安全危害的污点，如简单的类型或者已经经过足够多校验的污点，一律不要作为污点继续跟踪，也不要输出到最终结果，可根据经验进行判断是否属于攻击者可控内容。 \n"
        "7. 如果目标函数和传入的污点，不太可能造成安全问题，或者目标函数功能是没有危险的，也不需要跟踪，也不要输出到最终结果。\n\n"
