...
def resolve_external(
    ...
    prompt = (
        f"## 任务背景\n"
        f"当前需要处理一条“外部逃逸”传播记录。它表示：污点没有直接作为普通 callee 参数继续向下传播，"
        f"而是先从源函数内部流出，进入了一个后续可能被其他函数读取的共享对象、容器、字段别名或全局状态。"
        f"你的任务不是寻找同名变量，而是判断后续哪些真实函数重新读取并使用了这份逃逸出去的数据，"
        f"使得污点可以继续向下传播。\n\n"
        f"## 源函数\n"
        f"- 函数: {func.file}::{func.name}\n"
        f"- 源码绝对根目录: `{source_root}`\n"
        f"- 说明: `{func.file}` 是相对源码根目录的路径；如果需要使用 read/find 读取源码，请基于这个绝对根目录定位文件，不要基于当前工作目录拼接路径。\n\n"
        f"## 当前这条逃逸传播的结构化信息\n"
        f"- escape_kind: {prop.escape_kind or '(未指定)'}\n"
        f"  含义: 逃逸类型。通常表示污点是进入了容器(container)、全局状态(global)、字段别名(field_alias)等非局部位置。\n"
        f"- carrier: {prop.carrier or '(无)'}\n"
        f"  含义: 当前承载污点的对象、变量、节点、结构体实例或容器载体。后续读者函数通常不是直接读取 source_taint，而是读取这个 carrier 或它内部的字段/元素。\n"
        f"- escape_via: {prop.escape_via or '(无)'}\n"
        f"  含义: 让污点逃逸出去的动作或调用，例如插入容器、挂链、入队、保存到上下文、写入全局对象等。\n"
        f"- source_taint: {prop.source_taint_name or '(无)'}\n"
        f"  含义: 源函数里原始污点的名称或语义。\n"
        f"- target_taint: {prop.target_taint_name or '(无)'}\n"
        f"  含义: 逃逸后那份污点在外部对象中的承载语义，未必会在后续函数里以相同字面名字出现。\n"
        f"- description: {prop.description or '(无)'}\n"
        f"  含义: 对这次传播细节的补充说明，例如“只有某个 field 被污染”“污染的是容器中的某类元素”等。\n\n"
        f"## 你需要完成的判断\n"
        f"请结合上面的字段语义，用 v2_db 查找真正会重新读取这份逃逸污点的下游函数。重点不是做字符串匹配，"
        f"而是判断语义上是否读取了同一份共享数据，例如：\n"
        f"- 遍历容器后取出其中元素\n"
        f"- 读取 carrier 的字段、成员、别名或解引用结果\n"
        f"- 从全局/上下文对象中重新取出之前写入的数据\n"
        f"- 将这份数据继续作为参数、控制条件或危险操作输入使用\n\n"
        f"## 输出要求\n"
        f"只输出“确实读取到了这条逃逸污点”的真实函数；不要因为变量同名或类型接近就输出。"
        f"如果只能确认对象可能相关，但无法确认它真的读到了这条污点，则不要输出。\n\n"
        f"攻击面复核要求: 获取污点的下游函数的前提，都要根据函数的功能，根据污点值的含义，先判断污点以及逃逸的内容属于外部攻击者可控制的内容,以及攻击者可控的内容能够造成安全危害；对攻击者不可控的变量、进程内部状态、纯内部派生值以及其他无法控制的内容，以及不太可能造成安全危害的污点，如简单的类型或者已经经过足够多校验的污点，一律不要作为污点继续跟踪，也不要输出到最终结果，可根据经验进行判断是否属于攻击者可控内容。 \n"
