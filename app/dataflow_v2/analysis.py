...
class TaintAnalysisCallbacks(AnalysisCallbacks):
    ...
    def _build_prompt(self, func: FunctionRecord, body: str,
        ...
        return (
            ...
            f"对攻击者不可控的常量、编译期固定值、静态配置、进程内部状态、纯内部派生值以及其他无法控制的内容，以及不太可能造成安全危害的污点，一律不要作为污点继续跟踪，也不要输出到最终结果，可根据经验进行判断是否属于攻击者可控内容。\n\n"
            f"如果目标函数和传入的污点，不太可能造成安全问题，或者目标函数功能是没有危险的，也不需要跟踪，也不要输出到最终结果。\n\n"
            f"入口污点: {taint_desc}\n\n"
            f"## 前置校验链 (从根到本函数已累积)\n{pre_val_text}\n\n"
            f"## 函数体\n```c\n{body}\n```\n\n"
