    ...
    def _build_prompt(self, func: FunctionRecord, body: str,
        ...
        if taint_params.names == ["auto"] or taint_params.signature == "auto":
            taint_desc = ("自行分析（EA 未指定具体污点参数）。\n"
                          ...
                          "注意：只标注和跟踪外部攻击者可能控制的内容，不要标注攻击者无法控制的内部常量、编译期常量、静态配置、进程内部状态、纯内部派生值等其他外部攻击者无法控制的内容；"
                          "对确认攻击者可控的污点，即使本函数只是把它转发/透传给 callee，也必须报出对应 propagation，由下游决定是否成洞——本阶段不做漏洞判定。")
        else:
            taint_desc = (f"位置 {taint_params.positions} 签名 {taint_params.signature} "
                          f"名字 {taint_params.names}\n"
                          "注意：上游已传入的污点参数默认视为外部攻击者可控，除非你在本函数体内发现明确证据证明某参数实际是常量/编译期固定值/静态配置/进程内部状态等攻击者不可控内容，才将其剔除且不跟踪；"
                          "对确认攻击者可控的污点，必须继续标注并跟踪其流向 callee 的传播，不得因本函数无自身 sink 而清空 propagations。")
        ...
        return (
            ...
            f"攻击面复核要求: 无论上游是否已传入污点参数，都要先判断当前函数里哪些输入/变量真实属于外部攻击者可控制的内容。\n"
            f"- 攻击者不可控的内容（常量、编译期固定值、静态配置、进程内部状态、纯内部派生值）不要作为污点标注或跟踪。\n"
            f"- 攻击者可控的外部输入（网络报文、外部回调带入的参数、用户态/外部接口传入的数据等）必须继续跟踪，即使本函数只是把它转发给 callee。\n\n"
            f"重要：本阶段只判定\"污点是否攻击者可控、是否流向 callee\"，不判定\"本函数是否有漏洞\"。本函数自身有无危险是漏洞挖掘阶段的事；只要污点流向某个 callee，就必须按系统提示词报出对应 propagation，由下游决定是否成洞。转发/透传函数自身永远无 sink，正因如此更必须报 propagation 让下游跟入，不得因\"本函数没危险\"而清空 propagations。\n\n"
            f"入口污点: {taint_desc}\n\n"
            f"## 前置校验链 (从根到本函数已累积)\n{pre_val_text}\n\n"
            f"## 函数体\n```c\n{body}\n```\n\n"
